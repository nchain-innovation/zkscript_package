...
def test_msm_with_fixed_bases(scalars, bases, max_multipliers, index, rolling_option):
    ...
    for ix, multiplier in enumerate(max_multipliers):
        if ix != index:
            # Drop block for non-extracted scalar (#elements + zero marker)
            script += Script.parse_string(" ".join(["OP_DROP"] * (int(log2(multiplier)) * 4 + 1)))
        else:
            # Drop block for extracted scalar, conditional on whether it was rolled or not
            script += Script.parse_string(
                " ".join(["OP_DROP"] * (int(log2(multiplier)) * 2 * (2 - rolling_option) + 1))
            )
