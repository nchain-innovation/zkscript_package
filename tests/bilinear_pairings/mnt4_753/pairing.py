...
def test_pairing():
    ...
    output = mnt4_753_curve.pairing(P,Q)
    
    lambdas_Q_exp_miller_loop = [list(map(lambda s: s.to_list(),el)) for el in Q.get_lambdas(exp_miller_loop)]

    unlock = mnt4_753.single_pairing_input(
        P=P.to_list(),
        Q=Q.to_list(),
        lambdas_Q_exp_miller_loop=lambdas_Q_exp_miller_loop,
        miller_output_inverse=mnt4_753_curve.miller_loop_on_twisted_curve(P,Q,'quadratic').invert().to_list(),
    )
    
    # Check correct evaluation
    lock = mnt4_753.single_pairing(modulo_threshold=1,check_constant=True,clean_constant=True)
...
print('\nBegin tests pairing for MNT4-753 -------------\n')
...
print('\nPairing MNT4_753: all tests successful -------\n')