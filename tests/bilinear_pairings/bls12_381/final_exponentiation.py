...
from elliptic_curves.instantiations.bls12_381.bls12_381 import Fq12, bls12_381
from elliptic_curves.instantiations.bls12_381.final_exponentiation import easy_exponentiation, hard_exponentiation

from zkscript.bilinear_pairings.bls12_381.final_exponentiation import final_exponentiation
...
def test_easy_exponentiation_with_inverse_check():
    x = Fq12.generate_random_point()
    x_inverse = x.invert()
    x_cubic = deepcopy(x)
    # Swap between Fq12 and Fq12Cubic (only as a list of elements)
    x_cubic.x0.x0, x_cubic.x1.x1, x_cubic.x1.x0, x_cubic.x0.x2, x_cubic.x0.x1, x_cubic.x1.x2 = x_cubic.x0.x0, x_cubic.x0.x1, x_cubic.x0.x2, x_cubic.x1.x0, x_cubic.x1.x1, x_cubic.x1.x2

    z = easy_exponentiation(x)
    
    unlock =nums_to_script([q])
    unlock += generate_unlock(x_inverse)
    unlock += generate_unlock(x_cubic)
...
print('\nBegin tests final exponentation BLS12-381 -------------\n')
...
print('\nFinal exponentiation BLS12_381: all tests successful --\n')