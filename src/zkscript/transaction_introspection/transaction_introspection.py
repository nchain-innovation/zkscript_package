...
class TransactionIntrospection:
    ...
    def pushtx(
        ...
        Stack input:
            - stack:    [GROUP_ORDER_INT, Gx, 0220||Gx_bytes||02, .., data, ..]
            - altstack: []
        Stack output:
            - stack:    [GROUP_ORDER_INT, Gx, 0220||Gx_bytes||02, .., data, ..] or fail (
        ...
        # stack in:  [GROUP_ORDER_INT, Gx, 0220||Gx_bytes||02, .., data, ..]
        # stack out: [GROUP_ORDER_INT, Gx, 0220||Gx_bytes||02, .., data, ..,
        #               0220||Gx_bytes||02, 0x0220||Gx_bytes||02]
        out += (
            roll(position=-3, n_elements=1) if clean_constants else pick(position=-3, n_elements=1)
        )  # Move 0220||Gx_bytes||02
        out += Script.parse_string("OP_DUP")

        # stack out: [GROUP_ORDER_INT, Gx, 0220||Gx_bytes||02, .., data, ..,
        #               0220||Gx_bytes||02, 0x0220||Gx_bytes||02, h]
        ...
        # stack out: [GROUP_ORDER_INT, Gx, 0220||Gx_bytes||02, .., data, ..,
        #               0220||Gx_bytes||02, 0x0220||Gx_bytes||02, GROUP_ORDER_INT, (h + Gx) % GROUP_ORDER_INT]
        ...
        # stack out: [GROUP_ORDER_INT, Gx, 0220||Gx_bytes||02, .., data, ..,
        #               0220||Gx_bytes||02, 0x0220||Gx_bytes||02, s]
        out += int_sig_to_s_component(add_prefix=False)

        # stack out: [GROUP_ORDER_INT, Gx, 0220||Gx_bytes||02, .., data, ..,
        #               0220||Gx_bytes||02, Der(Gx,s)]
    ...
    def pushtx_bit_shift(
        ...
        Args:
            ...
            rolling_option (int): Whether to roll or pick `data`. Defaults to `True` (roll).
            is_sig_hash_preimage (bool): If `True`, the script expects `data` to be the sig_hash_preimage.
                Else, it expects it to be the double sha256 digest of the sig_hash_preimage (as a bytestring).
            is_checksigverify (bool): Whether to execute OP_CHECKSIGVERIFY or OP_CHECKSIG. Defaults
                to `True` (OP_CHECKSIGVERIFY).
            is_opcodeseparator (bool): Whether to prepend the signature verification with an
                OP_CODESEPARATOR (so that the code of PUSHTX is not signed). Defaults to `False`.
