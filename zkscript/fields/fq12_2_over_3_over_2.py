...
class Fq12:
	r"""
	F_q^12 as quadratic extension of F_q^6, which is built as cubic extension of F_q^2.
	
	F_q^12 = F_q^6[u] / u^2 - v, F_q^6 = F_q^2[v] / v^3 - NON_RESIDUE_OVER_FQ2
	"""

	def __init__(self, q: int, fq2, fq6, gammas_frobenius: Optional[list[list[int]]] = None):
		...
		# Fq6 implementation
		self.FQ6 = fq6
		# Gammas for the Frobenius - list of [gamma1,gamma2,...,gamma11] where gammai = [gammai1, .., gammai6], with gammaij = list of coefficients of NON_RESIDUE_OVER_FQ2.power(j * (q**i-1)//6)
	...
	def mul(self, take_modulo: bool, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None, is_constant_reused: Optional[bool] = None) -> Script:
		...
		compute_second_component = Script.parse_string('OP_2OVER OP_2OVER') + pick(position=9,nElements=2) + Script.parse_string('OP_2ROT OP_2ROT')		# Pick y1
		...
		compute_second_component += pick(position=15,nElements=4) + pick(position=21,nElements=2) + Script.parse_string('OP_2ROT OP_2ROT')				# Pick y0
		...
		compute_second_component += Script.parse_string('OP_TOALTSTACK OP_TOALTSTACK OP_TOALTSTACK OP_TOALTSTACK OP_TOALTSTACK OP_TOALTSTACK')
		...
		compute_first_component = roll(position=15,nElements=4) + roll(position=17,nElements=2) + Script.parse_string('OP_2ROT OP_2ROT')				# Roll x1
		...
		compute_first_component += Script.parse_string('OP_TOALTSTACK OP_TOALTSTACK OP_TOALTSTACK OP_TOALTSTACK OP_TOALTSTACK OP_TOALTSTACK')

		# After this, the stack is: firstComponent, altstack = [secondComponent]
		compute_first_component += fq6.mul(take_modulo=False,check_constant=False,clean_constant=False)
		compute_first_component += Script.parse_string('OP_FROMALTSTACK OP_FROMALTSTACK OP_FROMALTSTACK OP_FROMALTSTACK OP_FROMALTSTACK OP_FROMALTSTACK')
		if take_modulo:
			compute_first_component += fq6.add(take_modulo=True,check_constant=False,clean_constant=clean_constant,is_constant_reused=True)
		else:
		...
		if take_modulo:
			...
			out += Script.parse_string('OP_FROMALTSTACK OP_FROMALTSTACK OP_FROMALTSTACK OP_FROMALTSTACK OP_FROMALTSTACK OP_FROMALTSTACK')
	...
	def square(self, take_modulo: bool, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None, is_constant_reused: Optional[bool] = None) -> Script:
		...
		# After this, the stack is: a b c d e f (d*e), altstack = [sixthComponent, fifthComponent, fourthComponent]
		...
		# After this, the stack is: a b c d e f (d*e) f^2*xi, altstack = [sixthComponent, fifthComponent, fourthComponent]
		...
		# After this, the stack is: a b c d e f f^2*xi 2*[(d*e) + (a*c)], altstack = [sixthComponent, fifthComponent, fourthComponent]
		...
		# After this, the stack is: a b c d e f, altstack = [sixthComponent, fifthComponent, fourthComponent, 2*[f^2*xi 2*[(d*e) + (a*c)] + b^2]]
		...
		# After this, the stack is: a b c d e f 2*(e*f), altstack = [sixthComponent, fifthComponent, fourthComponent, thirdComponent]
		...
		# After this, the stack is: a b c d e f xi*[c^2 + 2*e*f], altstack = [sixthComponent, fifthComponent, fourthComponent, thirdComponent]
		...
		# After this, the stack is: a b c d e f xi*[c^2 + 2*e*f] 2*a*b, altstack = [sixthComponent, fifthComponent, fourthComponent, thirdComponent]
		...
		# After this, the stack is: a b c d e f, altstack = [sixthComponent, fifthComponent, fourthComponent, thirdComponent, xi*[c^2 + 2*e*f] + 2*a*b + d^2]
		...
		if take_modulo:
			...
			out += Script.parse_string('OP_FROMALTSTACK OP_2 OP_MUL OP_FROMALTSTACK OP_2 OP_MUL OP_FROMALTSTACK OP_2 OP_MUL')
			out += Script.parse_string('OP_FROMALTSTACK OP_2 OP_MUL OP_FROMALTSTACK OP_2 OP_MUL OP_FROMALTSTACK OP_2 OP_MUL')

		return out

	def conjugate(self, take_modulo: bool, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None, is_constant_reused: Optional[bool] = None) -> Script:
		'''
		Conjugate in F_q^12.
		Input parameters:
			- Stack: q .. X
	...
	def frobenius_odd(self, n: int, take_modulo: bool, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None, is_constant_reused: Optional[bool] = None) -> Script:
		r"""
		Frobenius for odd powers n = 2k + 1 in F_q^12.

		Then, Frobenius is computed via the isomorphism: F_q^12 \sim F_q^2[u,v] / (u^2 - v, v^3 - NON_RESIDUE_OVER_FQ2) \sim F_q^2[t] / (t^6 - NON_RESIDUE_OVER_FQ2)
		...
		a_conjugate += fq2.conjugate(take_modulo=take_modulo,check_constant=False,clean_constant=False,is_constant_reused=False)		# Conjugate a
		...
		b_conjugate_times_gamma12 += fq2.conjugate(take_modulo=False,check_constant=False,clean_constant=False)							# Conjugate b
		b_conjugate_times_gamma12 += nums_to_script(gammas[1])																			# gamma12
		b_conjugate_times_gamma12 += fq2.mul(take_modulo=take_modulo,check_constant=False,clean_constant=False,is_constant_reused=False)# Multiply
		...
		c_conjugate_gamma14 += fq2.conjugate(take_modulo=False,check_constant=False,clean_constant=False)								# Conjugate c
		c_conjugate_gamma14 += nums_to_script(gammas[3])																				# gamma14
		c_conjugate_gamma14 += fq2.mul(take_modulo=take_modulo,check_constant=False,clean_constant=False,is_constant_reused=False)		# Multiply
		c_conjugate_gamma14 += Script.parse_string('OP_2ROT OP_2ROT')																	# Bring e and f on top of the stack

		# After this, the stack is: Conjugate(a) [Conjugate(b) * gamma12] [Conjugate(c) * gamma14] [Conjugate(d) * gamma11] e f
		d_conjugate_gamma11 = roll(position=11,nElements=2)																				# Bring d on top of the stack
		d_conjugate_gamma11 += fq2.conjugate(take_modulo=False,check_constant=False,clean_constant=False)								# Conjugate d
		d_conjugate_gamma11 += nums_to_script(gammas[0])																				# gamma11
		d_conjugate_gamma11 += fq2.mul(take_modulo=take_modulo,check_constant=False,clean_constant=False,is_constant_reused=False)		# Multiply
		d_conjugate_gamma11 += Script.parse_string('OP_2ROT OP_2ROT')																	# Bring e and f on top of the stack

		# After this, the stack is: Conjugate(a) [Conjugate(b) * gamma12] [Conjugate(c) * gamma14] [Conjugate(d) * gamma11] [Conjugate(e) * gamma13] f
		e_conjugate_gamma13 = Script.parse_string('OP_2SWAP')																			# Bring e on top of the stack
		e_conjugate_gamma13 += fq2.conjugate(take_modulo=False,check_constant=False,clean_constant=False)								# Conjugate e
		e_conjugate_gamma13 += nums_to_script(gammas[2])																				# gamma13
		e_conjugate_gamma13 += fq2.mul(take_modulo=take_modulo,check_constant=False,clean_constant=False,is_constant_reused=False)		# Multiply
		e_conjugate_gamma13 += Script.parse_string('OP_2SWAP')																			# Bring f on top of the stack

		# After this, the stack is: Conjugate(a) [Conjugate(b) * gamma12] [Conjugate(c) * gamma14] [Conjugate(d) * gamma11] [Conjugate(e) * gamma13] [Conjugate(f) * gamma15]
		f_conjugate_gamma15 = fq2.conjugate(take_modulo=False,check_constant=False,clean_constant=False)								# Conjugate f
		f_conjugate_gamma15 += nums_to_script(gammas[4])																				# gamma15
		f_conjugate_gamma15 += fq2.mul(take_modulo=take_modulo,check_constant=False,clean_constant=clean_constant,is_constant_reused=is_constant_reused)				# Multiply

		out += a_conjugate + b_conjugate_times_gamma12 + c_conjugate_gamma14 + d_conjugate_gamma11 + e_conjugate_gamma13 + f_conjugate_gamma15

		return out

	def frobenius_even(self, n: int, take_modulo: bool, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None, is_constant_reused: Optional[bool] = None) -> Script:
		r"""
		...
		b_gamma22 += fq2.mul(take_modulo=take_modulo,check_constant=False,clean_constant=False,is_constant_reused=False)	# Multiply
		...
		c_gamma24 += fq2.mul(take_modulo=take_modulo,check_constant=False,clean_constant=False,is_constant_reused=False)	# Multiply
		...
		d_gamma21 += fq2.mul(take_modulo=take_modulo,check_constant=False,clean_constant=False,is_constant_reused=False)	# Multiply
		...
		e_gamma23 += fq2.mul(take_modulo=take_modulo,check_constant=False,clean_constant=False,is_constant_reused=False)	# Multiply
		...
		f_gamma25 += fq2.mul(take_modulo=take_modulo,check_constant=False,clean_constant=clean_constant,is_constant_reused=is_constant_reused)		# Multiply
