...
class Fq12Cubic:
	r"""
	F_q^12 as cubic extension of F_q^4, which is built as a quadratic extension of F_q^2. The NON_RESIDUE_OVER_FQ4 is specified by defining the method self.FQ4.mul_by_non_residue
	...
	def mul(self, take_modulo: bool, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None, is_constant_reused: Optional[bool] = None) -> Script:
		...
		# After this, the stack is: x0 x1 x2 y0 y1 y2 (x2*y2*NON_RESIDUE_OVER_FQ4), altstack = [(x2*y0) + (x1*y1) + (x0*y2)]
		...
		compute_second_component += fq4.mul_by_non_residue(take_modulo=False,check_constant=False,clean_constant=False)
		# After this, the stack is:  x0 x1 x2 y0 y1 y2 (x2*y2*NON_RESIDUE_OVER_FQ4) (x1*y0), altstack = [(x2*y0) + (x1*y1) + (x0*y2)]
		...
		compute_second_component += fq4.mul(take_modulo=False,check_constant=False,clean_constant=False)
		# After this, the stack is:  x0 x1 x2 y0 y1 y2, altstack = [(x2*y0) + (x1*y1) + (x0*y2), (x2*y2*NON_RESIDUE_OVER_FQ4) + (x1*y0) + (x0*y1)]
		...
		# After this, the stack is: x0 x1 y0 y2 (y1*x2), altstack = [(x2*y0) + (x1*y1) + (x0*y2), (x2*y2*NON_RESIDUE_OVER_FQ4) + (x1*y0) + (x0*y1)]
		...
		# After this, the stack is: x0 y0 [(y1*x2) + (y2*x1)] * NON_RESIDUE_OVER_FQ2, altstack = [(x2*y0) + (x1*y1) + (x0*y2), (x2*y2*s) + (x1*y0) + (x0*y1)]
		...
		compute_first_component += fq4.mul(take_modulo=False,check_constant=False,clean_constant=False)
		compute_first_component += Script.parse_string('OP_FROMALTSTACK OP_FROMALTSTACK OP_FROMALTSTACK OP_FROMALTSTACK')
		if take_modulo:
			compute_first_component += fq4.add(take_modulo=True,check_constant=False,clean_constant=clean_constant,is_constant_reused=True)
		else:
			compute_first_component += fq4.add(take_modulo=False,check_constant=False,clean_constant=False)
	...
	def square(self, take_modulo: bool, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None, is_constant_reused: Optional[bool] = None) -> Script:
		...
		compute_third_component += fq4.mul(take_modulo=False,check_constant=False,clean_constant=False)
		compute_third_component += Script.parse_string('OP_2') + fq4.fq_scalar_mul(take_modulo=False,check_constant=False,clean_constant=False)
		...
		compute_second_component += Script.parse_string('OP_2') + fq4.fq_scalar_mul(take_modulo=False,check_constant=False,clean_constant=False)
