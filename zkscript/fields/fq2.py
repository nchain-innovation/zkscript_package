...
class Fq2:
	'''
	Implementation of Quadratic Extension of base field. The modulus and the non_residue are specified when instantiating an object of this class.
	- MODULUS: an integer
	...
	Each function has a take_modulo boolean variable that lets us choose whether the coordinates of the result should be taken modulo q.
	
	This variable is introduced for future flexibility: it will allows us to reduce the number of opcodes.
	By looking at the number of multiplications in each formula, e.g. a * b + c * d requires only a final OP_MOD if a,b,c,d belong to Z_q; 
	on the other hand, a * b * c * d requires three modulo operations to avoid going above the limit for stack numbers: { [(a*b) % q] * [(c*d) % q] } % q.
	...
	def add(self, take_modulo: bool, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None, is_constant_reused: Optional[bool] = None) -> Script:
		...
		if take_modulo:
			...
			batched_modulo += Script.parse_string('OP_TUCK OP_MOD OP_OVER OP_ADD OP_OVER OP_MOD')										# Compute (x_0 + y_0) % q
			# After this, the stack is: [(x_0 + y_0) % q] (x_1 + y_1) q
			batched_modulo += Script.parse_string('OP_FROMALTSTACK OP_ROT')
			
			if is_constant_reused:
				# After this, the stack is: [(x_0 + y_0) % q] [(x_1 + y_1) % q]
				batched_modulo += Script.parse_string('OP_TUCK OP_MOD OP_OVER OP_ADD OP_OVER OP_MOD')									# Compute (x_1 + y_1) % q
			else:
				# After this, the stack is: [(x_0 + y_0) % q] q [(x_1 + y_1) % q]
				batched_modulo += Script.parse_string('OP_TUCK OP_MOD OP_OVER OP_ADD OP_SWAP OP_MOD')									# Compute (x_1 + y_1) % q
	...
	def negate(self, take_modulo: bool, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None, is_constant_reused: Optional[bool] = None) -> Script:
		...
		if take_modulo:
			...
			batched_modulo += Script.parse_string('OP_TUCK OP_MOD OP_OVER OP_ADD OP_OVER OP_MOD')									# Compute -x0 % q
			# After this, the stack is: [(x_0 + y_0) % q] (x_1 + y_1) q
			batched_modulo += Script.parse_string('OP_FROMALTSTACK OP_ROT')
			
			if is_constant_reused:
				# After this, the stack is: [-x0 % q] [-x1 % q]
				batched_modulo += Script.parse_string('OP_TUCK OP_MOD OP_OVER OP_ADD OP_OVER OP_MOD')								# Compute -x1 % q
			else:
				# After this, the stack is: [-x0 % q] q [-x1 % q]
				batched_modulo += Script.parse_string('OP_TUCK OP_MOD OP_OVER OP_ADD OP_SWAP OP_MOD')								# Compute -x1 % q
	...
	def scalar_mul(self, take_modulo: bool, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None, is_constant_reused: Optional[bool] = None) -> Script:
		...
		if take_modulo:
			...
			batched_modulo += Script.parse_string('OP_TUCK OP_MOD OP_OVER OP_ADD OP_OVER OP_MOD')									# Compute (x_0 * lambda) % q
			# After this, the stack is: [(x_0 * lambda) % q] (x_1 * lambda) q
			batched_modulo += Script.parse_string('OP_FROMALTSTACK OP_ROT')
			
			if is_constant_reused:
				# After this, the stack is: [(x_0 * lambda) % q] [-(x_1 * lambda) % q]
				batched_modulo += Script.parse_string('OP_TUCK OP_MOD OP_OVER OP_ADD OP_OVER OP_MOD')								# Compute (x_1 * lambda) % q
			else:
				# After this, the stack is: [(x_0 * lambda) % q] q [(x_1 * lambda) % q]
				batched_modulo += Script.parse_string('OP_TUCK OP_MOD OP_OVER OP_ADD OP_SWAP OP_MOD')								# Compute (x_1 * lambda) % q
	...
	def mul(self, take_modulo: bool, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None, is_constant_reused: Optional[bool] = None) -> Script:
		...
		Example:
			take_modulo = False:
				x_0 x_1 y_0 y_1 [mul] --> (x_0 * y_0 - x_1 * y_1) (x_0 * y_1 + x_1 * y_0)				where X = x_0 + x_1 u, Y = y_0 + y_1 u 
			take_modulo = True:
				x_0 x_1 y_0 y_1 [mul] --> [(x_0 * y_0 - x_1 * y_1) % q] [(x_0 * y_1 + x_1 * y_0) % q] 	where X = x_0 + x_1 u, Y = y_0 + y_1 u 
		'''
		...
		if self.NON_RESIDUE == -1:
			...
			firstComponent += nums_to_script([self.NON_RESIDUE]) + Script.parse_string('OP_MUL OP_ADD')									# Compute (x_0 * y_0 + x_1 * y_1 * NON_RESIDUE)
	...
	def square(self, take_modulo: bool, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None, is_constant_reused: Optional[bool] = None) -> Script:
		...
		Example:
			take_modulo = False:
				x_0 x_1 [square] --> (x_0^2 - x_1^2) (2 * x_0 * x_1) 				where X = x_0 + x_1 u, Y = y_0 + y_1 u 
			take_modulo = True:
				x_0 x_1 [square] --> [(x_0^2 - x_1^2) % q] [(2 * x_0 * x_1) % q] 	where X = x_0 + x_1 u, Y = y_0 + y_1 u 
		'''
		...
		if self.NON_RESIDUE == -1:
			...
			out += Script.parse_string('OP_SUB OP_2SWAP OP_ADD OP_MUL')													# Compute (x0 - x1), compute (x0 + x1), multiply
			...
			if take_modulo:
				...
				out += fetch_q + Script.parse_string('OP_TUCK OP_MOD OP_OVER OP_ADD OP_OVER OP_MOD') 					# Compute (x_0^2 - x_1^2) % q
				# After this, the stack is: [(x0^2 - x1^2) % q] (2x0x1) q
				out += Script.parse_string('OP_2SWAP OP_MUL OP_2 OP_MUL OP_ROT')
				
				if is_constant_reused:
					# After this, the stack is: [(x0^2 - x1^2) % q] q [(2x0x1) % q]
				...
				out += Script.parse_string('OP_ROT OP_ROT OP_MUL OP_2 OP_MUL')											# Compute 2 * x_0 * x_1 and place it on top of the stack
		else:
			...
			out += Script.parse_string('OP_DUP') + nums_to_script([self.NON_RESIDUE]) + Script.parse_string('OP_MUL OP_MUL')
			...
			if take_modulo:
				...
				batched_modulo += Script.parse_string('OP_TUCK OP_MOD OP_OVER OP_ADD OP_OVER OP_MOD')									
				# After this, the stack is: [firstComponent % q] secondComponent q
				batched_modulo += Script.parse_string('OP_FROMALTSTACK OP_ROT')
				
				if is_constant_reused:
					# After this, the stack is: [firstComponent % q] q [secondComponent % q]
					batched_modulo += Script.parse_string('OP_TUCK OP_MOD OP_OVER OP_ADD OP_OVER OP_MOD')
				else:
					# After this, the stack is: [firstComponent % q] [secondComponent % q]
					batched_modulo += Script.parse_string('OP_TUCK OP_MOD OP_OVER OP_ADD OP_SWAP OP_MOD')
	...
	def add_three(self, take_modulo: bool, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None, is_constant_reused: Optional[bool] = None) -> Script:
		'''
		...
		Example:
			take_modulo = False:
				x_0 x_1 y_0 y_1 z_0 z_1 [add] --> (x_0 + y_0 + z_0) (x_1 + y_1 + z_1)				where X = x_0 + x_1 u, Y = y_0 + y_1 u, Z = z_0 + z_1 u
			take_modulo = True:
				x_0 x_1 y_0 y_1 z_0 z_1 [add] --> [(x_0 + y_0 + z_0) % q] [(x_1 + y_1 + z_1) % q]	where X = x_0 + x_1 u, Y = y_0 + y_1 u, Z = z_0 + z_1 u
		'''
		...
		if take_modulo:
			...
			out += fetch_q + Script.parse_string('OP_TUCK OP_MOD OP_OVER OP_ADD OP_OVER OP_MOD')
			...
			if is_constant_reused:
				# After this, the stack is: [(x0 + y0 + z0) % q] q [(x1+y1+z1) % q]
				out += Script.parse_string('OP_OVER OP_MOD OP_OVER OP_ADD OP_OVER OP_MOD')
			else:
				# After this, the stack is: [(x0 + y0 + z0) % q] [(x1+y1+z1) % q]
	...
	def mul_by_u(self, take_modulo: bool, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None, is_constant_reused: Optional[bool] = None) -> Script:
		'''
		...
		if take_modulo:
			batchedModulo = Script()
			
			assert(clean_constant != None and is_constant_reused != None)
			if clean_constant:
				fetchQ = Script.parse_string('OP_DEPTH OP_1SUB OP_ROLL')
			else:
				fetchQ = Script.parse_string('OP_DEPTH OP_1SUB OP_PICK')

			# After this, the stack is: [x1*NON_RESIDUE % q] x0 q
			batchedModulo += Script.parse_string('OP_TUCK OP_MOD OP_OVER OP_ADD OP_OVER OP_MOD')
			batchedModulo += Script.parse_string('OP_ROT OP_ROT')
			
			if is_constant_reused:
				batchedModulo += Script.parse_string('OP_TUCK OP_MOD OP_OVER OP_ADD OP_OVER OP_MOD')
			else:
				batchedModulo += Script.parse_string('OP_TUCK OP_MOD OP_OVER OP_ADD OP_SWAP OP_MOD')

			out += fetchQ + batchedModulo
		else:
	...
	def mul_by_one_plus_u(self, take_modulo: bool, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None,is_constant_reused: Optional[bool] = None) -> Script:
		'''
		...
		if self.NON_RESIDUE == -1:
			...
			if self.NON_RESIDUE > 0:
				# After this, the stack is: x0 + x1 * NON_RESIDUE, altstack = [x0 + x1]
				out += nums_to_script([self.NON_RESIDUE]) + Script.parse_string('OP_MUL OP_ADD')						# Compute (x_0 + x_1 * NON_RESIDUE)
			else:
				# After this, the stack is: x0 + x1 * NON_RESIDUE, altstack = [x0 + x1]
				out += Script.parse_string(str(abs(self.NON_RESIDUE)) + ' OP_NEGATE OP_MUL OP_ADD')					# Compute (x_0 + x_1 * NON_RESIDUE)
