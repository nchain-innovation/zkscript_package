...
# Utility scripts
from zkscript.util.utility_scripts import roll, pick, nums_to_script
...
class EllipticCurveFq:
	...
	def point_addition(self, take_modulo: bool, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None) -> Script:
		'''
		NOTE:
		When using this function, we need to be sure that P != Q. 
		...
		Assumption on parameters:
			...
			- lambda is the gradient of the line through P and Q, passed as an integers (minimally encoded, little endian)
			- P != Q
			- P != -Q
		...
		lambda_different_points += Script.parse_string('OP_TUCK OP_MUL')							# Compuet lambda *(xP - xQ)
		lambda_different_points += Script.parse_string('OP_FROMALTSTACK OP_SUB')					# Compute lambda *(xP - xQ) - (yP - yQ)
		...
		compute_coordinates += Script.parse_string('OP_2SWAP OP_TOALTSTACK OP_TUCK')				# Swap xP yP, place yP on altstack, duplicate xP
		...
		# If needed, mod out
		# After this, the stack is: x_(P+Q) y_(P+Q) q, altstack = [(lambda *(xP - xQ) - (yP - yQ) == 0)] with the coefficients in Fq (if executed)
		if take_modulo:
			out += batched_modulo

		check_lambda = Script.parse_string('OP_FROMALTSTACK')
		check_lambda += Script.parse_string('OP_OVER OP_MOD OP_OVER OP_ADD OP_SWAP OP_MOD')											# Mod out lambda *(xP - xQ) - (yP - yQ)
	...
	def point_doubling(self, take_modulo: bool, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None) -> Script:
		...
		# If needed, mod out
		# After this, the stack is: x_(P+Q) y_(P+Q) q, altstack = [lambda * 2yP == 3xP^2] with the coefficients in Fq (if executed)
		if take_modulo:
			out += batched_modulo

		check_lambda = Script.parse_string('OP_FROMALTSTACK')
		check_lambda += Script.parse_string('OP_OVER OP_MOD OP_OVER OP_ADD OP_SWAP OP_MOD')												# Mod out lambda * (yP - yQ) - (xP- xQ)
		check_lambda += Script.parse_string('OP_0 OP_EQUALVERIFY')
