...
class EllipticCurveFq:
	...
	def point_addition(self, take_modulo: bool, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None) -> Script:
		...
		lambda_different_points += Script.parse_string('OP_OVER OP_5 OP_PICK OP_MUL OP_3 OP_PICK OP_ADD')							# compute lambda x_Q + y_P
		...
		# If needed, mod out
		# After this, the stack is: x_(P+Q) y_(P+Q) q, altstack = [(lambda * (yP - yQ) - (xP- xQ) == 0)] with the coefficients in Fq (if executed)
		if take_modulo:
			out += batched_modulo
	...
	def point_doubling(self, take_modulo: bool, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None) -> Script:
		...
		if curve_a != 0:
			lambda_equal_points += nums_to_script([curve_a]) + Script.parse_string('OP_ADD')												# Compute 3 x_P^2 + a if a != 0
