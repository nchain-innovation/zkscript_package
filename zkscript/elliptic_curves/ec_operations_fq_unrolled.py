...
class EllipticCurveFqUnrolled:
	...
	def unrolled_multiplication(self, max_multiplier: int, modulo_threshold: int, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None) -> Script:
		'''
		...
		[lambdas,a] is the list constructed starting from:
			...
			- lambdas, which is the list where:
				...
				- lambdas[i] = [gradient needed to compute 2T, gradient needed to compute T + P] if exp_a[i] = 1, where T is the intermediate value of the computation
		The list [lambdas,a] is constructed as follows: set a = (a0,a1,...,a_N), where a = sum_i 2^i ai and M = log_2(max_multiplier).
		...
		The meaning of the list is the following:
			...
			- if it starts with OP_1:
				...
				- if after the doubling there is OP_1, then execute the addition, otherwise go to the next iteration of the loop.

		MODULO OPERATIONS:

		As we are carrying out EC operation over the base field, each element is a single number. The formula for EC point doubling/addition is:
		
		x_(P+Q) = lambda^2 - x_P - x_Q
		y_(P+Q) = -y_P + (x_(P+Q) - x_P) * lambda

		where lambda is the gradient of the line through P and Q. Then, we have (with q exchanged for current_size in future steps)
		...
		Hence, we at every step we check that the next operation doesn't make log_2(q) + log_2(q^2 + 3q + 1) > modulo_threshold.
		'''
		...
		size_q = ceil(log(self.MODULUS,2))
		current_size = size_q

		# After this, the stack is: marker_a_is_zero P aP
		for i in range(int(log(max_multiplier,2))-1,-1,-1):
			# This is an approximation, but I'm quite sure it works. We always have to take into account both operations because we don't know which ones are going to be executed.
			...
			out += Script.parse_string('OP_2ROT')				# Roll marker to decide whether to excute the loop and the auxiliary data
			out += Script.parse_string('OP_IF')					# Check marker for executing iteration; if we enter here, the stack is: P T lambda_2T
			out += Script.parse_string('OP_ROT OP_ROT')			# Roll T
			out += ec_over_fq.point_doubling(take_modulo=take_modulo,check_constant=False,clean_constant=False)		# Compute 2T
			out += Script.parse_string('OP_2ROT')				# Roll marker for addition and auxiliary data addition
			out += Script.parse_string('OP_IF')					# Check marker for +P; if we enter here, the stack is: P 2T lambda_(2T+P)
			out += Script.parse_string('OP_ROT OP_ROT')			# Roll 2T
			out += pick(position=4,nElements=2)					# Pick P
			out += ec_over_fq.point_addition(take_modulo=take_modulo,check_constant=False,clean_constant=False)		# Compute 2T + P
			...
			out += Script.parse_string('OP_DROP')				# Drop useless data (if marker_doubling = False => auxiliary_data, if marker_addition = False => auxiliary_data_addition)
	...
	def unrolled_multiplication_input(self, P: list[int], a: int, lambdas: list[list[list[int]]], max_multiplier: int, load_modulus = True) -> Script:
		'''
		This function returns the input script needed to execute the unrolled multiplication script above.

		lambdas is the sequence of lambdas as they are computed by executing the multiplication.
		If exp_a is the binary representation of a, then len(lambdas) = len(exp_a)-1, because the last element of exp_a (the most significant bit) is not used.
		...
		if a == 0:
			...
			for j in range(len(lambdas)-1,-1,-1):
					if exp_a[-j-2] == 1:
						...
						out += Script.parse_string('OP_0 OP_0') + nums_to_script(lambdas[j][0]) + Script.parse_string('OP_1')
