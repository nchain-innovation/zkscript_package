...
class EllipticCurveFq2:
	...
	def point_addition(self, take_modulo: bool, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None) -> Script:
		'''
		Input Parameters:
			- Stack: q .. <lambda> P Q
			- Altstack: []
		...
		lambda_different_points += fq2.subtract(take_modulo=False,check_constant=False,clean_constant=False)									# Compute y_P - y_Q
		lambda_different_points += fq2.subtract(take_modulo=True,check_constant=False,clean_constant=False,is_constant_reused=False)				# Compute lambda (x_P - x_Q) - (y_P - y_Q)
		...
		compute_x_coordinate += fq2.square(take_modulo=False,check_constant=False,clean_constant=False)											# Compute lambda^2
		compute_x_coordinate += Script.parse_string('OP_2SWAP')																					# Roll x_Q
		compute_x_coordinate += pick(position=7,nElements=2)																					# Pick x_P
		compute_x_coordinate += fq2.add(take_modulo=False,check_constant=False,clean_constant=False)											# Compute (x_P + x_Q)
		compute_x_coordinate += fq2.subtract(take_modulo=take_modulo,check_constant=False,clean_constant=False,is_constant_reused=False)			# Compute lambda^2 - (x_P + x_Q))
		...
		compute_y_coordinate += fq2.subtract(take_modulo=False,check_constant=False,clean_constant=False)										# Compute (x_P - x_(P+Q))
		compute_y_coordinate += fq2.mul(take_modulo=False,check_constant=False,clean_constant=False)											# Compute lambda (x_P - x_(P+Q))
		compute_y_coordinate += Script.parse_string('OP_2ROT')																					# Roll y_P
		compute_y_coordinate += fq2.subtract(take_modulo=take_modulo,check_constant=False,clean_constant=clean_constant,is_constant_reused=False)	# Compute lambda (x_P - x_(P+Q)) - y_P

		out += lambda_different_points + compute_x_coordinate + compute_y_coordinate
		
		return out

	def point_doubling(self, take_modulo: bool, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None) -> Script:
		'''
		Input Parameters:
			- Stack: q .. <lambda> P
			- Altstack: []
		...
		lambda_equal_points += fq2.scalar_mul(take_modulo=False,check_constant=False,clean_constant=False)									# Compute 2y_P
		lambda_equal_points += pick(position=7,nElements=2)																					# Pick lambda
		lambda_equal_points += fq2.mul(take_modulo=False,check_constant=False,clean_constant=False)											# Compute 2 lambda y_P
		lambda_equal_points += pick(position=5,nElements=2)																					# Pick x_P
		lambda_equal_points += fq2.square(take_modulo=False,check_constant=False,clean_constant=False)										# Compute x_P^2
		lambda_equal_points += Script.parse_string('OP_3')
		lambda_equal_points += fq2.scalar_mul(take_modulo=False,check_constant=False,clean_constant=False)									# Compute 3 x_P^2
		if not all([el == 0 for el in curve_a]):
			lambda_equal_points += nums_to_script(curve_a)
			lambda_equal_points += fq2.add(take_modulo=False,check_constant=False,clean_constant=False,is_constant_reused=False)				# Compute 3 x_P^2 + a if a != 0
		lambda_equal_points += fq2.subtract(take_modulo=True,check_constant=False,clean_constant=False,is_constant_reused=False)				# Compute 2 lambda y_P - 3 x_P^2
		...
		compute_x_coordinate += fq2.scalar_mul(take_modulo=False,check_constant=False,clean_constant=False)									# Compute 2 x_P
		compute_x_coordinate += fq2.subtract(take_modulo=take_modulo,check_constant=False,clean_constant=False,is_constant_reused=False)	# Compute lambda^2 - (2 x_P)
		...
		compute_y_coordinate += fq2.mul(take_modulo=False,check_constant=False,clean_constant=False)										# Compute lambda (x_P - x_(2P))
		compute_y_coordinate += roll(position=5,nElements=2)																				# Roll y_P
		compute_y_coordinate += fq2.subtract(take_modulo=take_modulo,check_constant=False,clean_constant=clean_constant,is_constant_reused=False)	# Compute lambda (x_P - x_(2P)) - y_P
