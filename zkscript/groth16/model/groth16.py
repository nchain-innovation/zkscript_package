...
from zkscript.util.utility_scripts import roll, nums_to_script
...
# Pairing
from zkscript.bilinear_pairings.model.model_definition import PairingModel
...
class Groth16(PairingModel):
	...
	def groth16_verifier(
		...
		Input:
			Stack: q .. inverse_miller_loop_triple_pairing lambdas_pairing A B C lambda[sum_(i=0)^(l-1) a_i * gamma_abc[i], a_l * gamma_abc[l]] .. lambda[gamma_abc[0], a_1 * gamma_abc[1]] a_1 lambdas[a_1,gamma_abc[1]] .. a_l lambdas[a_l,gamma_abc[l]]
			Altstack: []
		Output:
			Verify ZKP equation

		Here:
			- a_i lambdas[a_i,gamma_abc[i]] is the input required to execute unrolled_multiplication from EllipticCurveFqUnrolled (except for gamma_abc[i], which is hard coded into the script)
			- lambda[sum_(i=0)^(j-1) a_i * gamma_abc[i], a_j * gamma_abc[j]] is the gradient through a_j * gamma_abc[j] and sum_(i=0)^(j-1) a_i * gamma_abc[i] to compute their sum
			- lambdas_pairing are the lambdas needed to execute the function self.triple_pairing() (from the Pairing class) to compute the triple pairing on the LHS of equation (*)
		'''
		...
		'''
		After this:
			- the stack is: q .. inverse_miller_loop_triple_pairing lambdas_pairing A B C lambda[sum_(i=0)^(l-1) a_i * gamma_abc[i], a_l * gamma_abc[l]] .. lambda[gamma_abc[0], a_1 * gamma_abc[1]] gamma_abc[0]
			- the altstack: a_l * gamma_abc[l] ... a_1 * gamma_abc[1]
		'''
		for i in range(n_pub,-1,-1):
			# After this, the top of the stack is: a_(i-1) lambdas[a_(i-1),gamma_abc[i-1]], altstack = [..., a_i * gamma_abc[i]]
		...
		# After this, the stack is: q .. lambdas_pairing inverse_miller_loop_triple_pairing A B C sum_(i=0)^l a_i * gamma_abc[i]
		...
		# After this, the stack is: q .. lambdas_pairing inverse_miller_loop_triple_pairing A sum_(i=0)^l a_i * gamma_abc[i] C B gamma delta
		...
		# After this, the top of the stack is: [e(A,B) * e(sum_(i=0)^(l) a_i * gamma_abc[i], gamma) * e(C, delta) ?= alpha_beta]
	...
	def groth16_verifier_unlock(
		...
		r'''
		Function to generate unlocking script for groth16_verifier.
			- pub: list of public statements
			- (A,B,C): zk-proof, elements passed as their list of coordinates
			- lambdas_B_exp_miller_loop: gradients needed to compute val * B, see also unrolled_multiplication in EllipticCurveFqUnrolled (val is the value over which we compute the Miller loop)
