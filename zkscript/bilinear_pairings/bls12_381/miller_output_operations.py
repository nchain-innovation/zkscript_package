...
from zkscript.fields.fq12_3_over_2_over_2 import Fq12Cubic as Fq12CubicScriptModel
from zkscript.bilinear_pairings.bls12_381.fields import fq2_script, fq4_script
...
class MillerOutputOperations(Fq12CubicScriptModel):
	...
	def line_eval_times_eval(self, take_modulo: bool, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None, is_constant_reused: Optional[bool] = None) -> Script:
		'''
		Multiplication of sparse by sparse in Fq^12 as a cubic extension. Sparse (M twist) means: a + bs + cr^2 in Fq^12 = Fq^4[r] / (r^3 - s) = F_q^2[s,r] / (r^3 - s, s^2 - xi), a,c are in Fq^2, b is in Fq
		Input parameters:
			- Stack: q .. X Y
			- Altstack: []
		Output:
		...
		# After this, the stack is: # After this, the stack is: a1 b1 a2 b2, altstack = [fifthComponent, fourthComponent, c1*c2]
		...
		# After this, the stack is: # After this, the stack is: a1 b1 a2 b2 (a1*b2), altstack = [fifthComponent, fourthComponent, thirdComponent]
		...
		# After this, the stack is: # After this, the stack is: a1 b1 a2 b2, altstack = [fifthComponent, fourthComponent, thirdComponent, a1*b2 + a2*b1]
		...
		# After this, the stack is: (a1*a2 + (b2*b1*xi)), altstack = [fifthComponent, fourthComponent, thirdComponent, secondComponent]
		...
		compute_first_component += fq2.mul(take_modulo=False,check_constant=False,clean_constant=False)	# Compute a1*a2
		# After this, the stack is: (a1*a2 + (b2*b1*xi))_0, altstack = [fifthComponent, fourthComponent, thirdComponent, secondComponent, (a1*a2 + (b2*b1*xi))_1]
		...
		if take_modulo:
			...
			compute_first_component += Script.parse_string('OP_TUCK OP_MOD OP_OVER OP_ADD OP_OVER OP_MOD')
			compute_first_component += Script.parse_string('OP_FROMALTSTACK OP_ROT')
			compute_first_component += Script.parse_string('OP_TUCK OP_MOD OP_OVER OP_ADD OP_OVER OP_MOD')
		else:
		...
		out += compute_fifth_component + compute_fourth_component + compute_third_component + compute_second_component + compute_first_component
		...
		if take_modulo:
			...
			out += Script.parse_string('OP_FROMALTSTACK OP_FROMALTSTACK OP_FROMALTSTACK OP_FROMALTSTACK OP_FROMALTSTACK OP_FROMALTSTACK OP_FROMALTSTACK OP_FROMALTSTACK')

		return out
	
	def miller_loop_output_times_eval(self, take_modulo: bool, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None, is_constant_reused: Optional[bool] = None) -> Script:
		'''
		Multiplication of dense by sparse in Fq^12 as a cubic extension.
		Sparse means: a + bs + cr^2 in Fq^12 = Fq^4[r] / (r^3 - s) = F_q^2[s,r] / (r^3 - s, s^2 - xi), a,c are in Fq^2 and b in Fq, as when evaluating line functions
		...
		# After this, the stack is: a1 b1 c1 d1 e1 f1 a2 b2 c2, altstack = [sixthComponent, (a1*c2) + (a2*e1) + b2*f1*xi]
		...
		# After this, the stack is: a1 b1 c1 d1 e1 f1 a2 b2 c2 (c1*b2), altstack = [sixthComponent, fifthComponent]
		...
		# After this, the stack is: a1 b1 c1 d1 e1 f1 a2 b2 c2 (c1*b2) (d1*a2), altstack = [sixthComponent, fifthComponent]
		compute_fourth_component += pick(position=12,nElements=2)		# Pick d1
		compute_fourth_component += pick(position=8,nElements=2)		# Pick a2
		compute_fourth_component += fq2.mul(take_modulo=False,check_constant=False,clean_constant=False)

		# After this, the stack is: a1 b1 c1 d1 f1 a2 b2 c2, altstack = [sixthComponent, fifthComponent, (c1*b2) + (d1*a2) + (e1*c2)]
		...
		# After this, the stack is: a1 b1 c1 d1 f1 a2 b2 c2 (c1*a2), altstack = [sixthComponent, fifthComponent, fourthComponent]
		compute_third_component = pick(position=10,nElements=2)			# Pick c1
		compute_third_component += pick(position=6,nElements=2)			# Pick a2
		compute_third_component += fq2.mul(take_modulo=False,check_constant=False,clean_constant=False)

		# After this, the stack is: a1 b1 c1 d1 f1 a2 b2 c2 (c1*a2) (b2*d1), altstack = [sixthComponent, fifthComponent, fourthComponent]
		...
		# After this, the stack is: a1 b1 c1 d1 a2 b2 c2, altstack = [sixthComponent, fifthComponent, fourthComponent, (c1*a2) + [(b2*d1) + (c2*f1)]*xi]
		...
		# After this, the stack is: a1 b1 c1 d1 a2 b2 c2 (a1*b2), altstack = [sixthComponent, fifthComponent, fourthComponent, thirdComponent]
		...
		# After this, the stack is: a1 b1 c1 d1 a2 b2 c2 (a1*b2) (b1*a2), altstack = [sixthComponent, fifthComponent, fourthComponent, thirdComponent]
		...
		# After this, the stack is:  a1 b1 d1 a2 b2 c2, altstack = [sixthComponent, fifthComponent, fourthComponent, thirdComponent, (a1*b2)+  (b1*a2) + (c1*c2)]
		...
		# After this, the stack is: a1 b1 a2 b2 (c2*d1), altstack = [sixthComponent, fifthComponent, fourthComponent, thirdComponent, secondComponent]
		...
		# After this, the stack is: a1 a2 [(c1*c2) + (b1*b2)]*xi, altstack = [sixthComponent, fifthComponent, fourthComponent, thirdComponent, secondComponent]
		...
		out += compute_sixth_component + compute_fifth_component + compute_fourth_component + compute_third_component + compute_second_component + compute_first_component
		...
		if take_modulo:
			...
			out += Script.parse_string('OP_FROMALTSTACK OP_FROMALTSTACK OP_FROMALTSTACK OP_FROMALTSTACK OP_FROMALTSTACK OP_FROMALTSTACK OP_FROMALTSTACK OP_FROMALTSTACK OP_FROMALTSTACK OP_FROMALTSTACK')

		return out

	def line_eval_times_eval_times_eval(self, take_modulo: bool, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None, is_constant_reused: Optional[bool] = None) -> Script:
		'''
		Multiplication of sparse by somewhat sparse in Fq^12 as a cubic extension.
		Sparse means: a + bs + cr in Fq^12 = Fq^4[r] / (r^3 - s) = F_q^2[s,r] / (r^3 - s, s^2 - xi), a,c are in Fq^2, b is in fq
		...
		# After this, the stack is: a1 b1 c1 a2 b2 c2 e2 (b1*c2), altstack = [sixthComponent, fifthComponent, fourthComponent]
		compute_third_component = Script.parse_string('OP_2OVER')		# Pick c2
		compute_third_component += pick(position=12,nElements=1)		# Pick b1
		compute_third_component += fq2.scalar_mul(take_modulo=False,check_constant=False,clean_constant=False)

		# After this, the stack is: a1 b1 c1 a2 b2 c2, altstack = [sixthComponent, fifthComponent, fourthComponent, ((b1*c2) + (c1*e2)) * xi]
		...
		# After this, the stack is: a1 b1 c1 a2 b2 c2 (a1*b2), altstack = [sixthComponent, fifthComponent, fourthComponent, thirdComponent]
		...
		# After this, the stack is: a1 b1 c1 a2 b2 c2, altstack = [sixthComponent, fifthComponent, fourthComponent, thirdComponent, a1*b2 + a2*b1]
		...
		# After this, the stack is: a1 b1 a2 b2 (c1*c2), altstack = [sixthComponent, fifthComponent, fourthComponent, thirdComponent, secondComponent]
	...
	def line_eval_times_eval_times_eval_times_eval(self,take_modulo: bool, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None, is_constant_reused: Optional[bool] = None) -> Script:
		'''
		...
		compute_sixth_component += fq2.add_three(take_modulo=False,check_constant=False,clean_constant=False) + fq2.add(take_modulo=False,check_constant=False,clean_constant=False)
		compute_sixth_component += Script.parse_string('OP_TOALTSTACK OP_TOALTSTACK')
		...
		# After this, the stack is: a1 b1 c1 d1 e1 a2 b2 c2 d2 e2 (d1*a2) ((c1*c2) + (e1*b2) + (b1*e2)) * xi, altstack = [sixthComponent]
		...
		# After this, the stack is: a1 b1 c1 d1 e1 a2 b2 c2 d2 e2 (d1*a2) (((c1*c2) + (e1*b2) + (b1*e2)) * xi) (a1*d2), altstack = [sixthComponent]
		compute_fifth_component += pick(position=23,nElements=2)	# Pick a1
		compute_fifth_component += pick(position=9,nElements=2)	# Pick d2
		compute_fifth_component += fq2.mul(take_modulo=False,check_constant=False,clean_constant=False)

		# After this, the stack is: a1 b1 c1 d1 e1 a2 b2 c2 d2 e2, altstack = [sixthComponent, (d1*a2) + (((c1*c2) + (e1*b2) + (b1*e2)) * xi) + (a1*d2)]
		compute_fifth_component += fq2.add_three(take_modulo=False,check_constant=False,clean_constant=False)
		compute_fifth_component += Script.parse_string('OP_TOALTSTACK OP_TOALTSTACK')
		...
		# After this, the stack is: a1 b1 c1 d1 e1 a2 b2 c2 d2 e2 (c1*a2) (d1*d2), altstack = [sixthComponent, fifthComponent]
		...
		# After this, the stack is: a1 b1 c1 d1 e1 a2 b2 c2 d2 e2 (c1*a2) (d1*d2) (e1*e2*xi), altstack = [sixthComponent, fifthComponent]
		...
		compute_fourth_component += fq2.mul(take_modulo=False,check_constant=False,clean_constant=False)
		compute_fourth_component += fq2.mul_by_non_residue(take_modulo=False,check_constant=False,clean_constant=False)

		# After this, the stack is: a1 b1 c1 d1 e1 a2 b2 c2 d2 e2 (c1*a2) (d1*d2) (e1*e2*xi) (a1*c2), altstack = [sixthComponent, fifthComponent]
		...
		# After this, the stack is: a1 b1 c1 d1 e1 a2 b2 c2 d2 e2, altstack = [sixthComponent, fifthComponent, (c1*a2) + (d1*d2) + (e1*e2*xi) + (a1*c2)]
		compute_fourth_component += fq2.add_three(take_modulo=False,check_constant=False,clean_constant=False)
		compute_fourth_component += fq2.add(take_modulo=False,check_constant=False,clean_constant=False)
		compute_fourth_component += Script.parse_string('OP_TOALTSTACK OP_TOALTSTACK')
		...
		# After this, the stack is: a1 b1 c1 d1 e1 a2 b2 c2 d2 e2 (c1*b2), altstack = [sixthComponent, fifthComponent, fourthComponent]
		compute_third_component = pick(position=15,nElements=2)	# Pick c1
		compute_third_component += pick(position=9,nElements=2)	# Pick b2
		compute_third_component += fq2.mul(take_modulo=False,check_constant=False,clean_constant=False)

		# After this, the stack is: a1 b1 c1 d1 e1 a2 b2 c2 d2 e2 (c1*b2) (d1*e2), altstack = [sixthComponent, fifthComponent, fourthComponent]
		compute_third_component += pick(position=15,nElements=2)	# Pick d1
		compute_third_component += pick(position=5,nElements=2)	# Pick e2
		compute_third_component += fq2.mul(take_modulo=False,check_constant=False,clean_constant=False)

		# After this, the stack is: a1 b1 c1 d1 e1 a2 b2 c2 d2 e2 (c1*b2) (d1*e2) (e1*d2), altstack = [sixthComponent, fifthComponent, fourthComponent]
		compute_third_component += pick(position=15,nElements=2)	# Pick e1
		compute_third_component += pick(position=9,nElements=2)	# Pick d2
		compute_third_component += fq2.mul(take_modulo=False,check_constant=False,clean_constant=False)

		# After this, the stack is: a1 b1 c1 d1 e1 a2 b2 c2 d2 e2 (c1*b2) (d1*e2) (e1*d2) (b1*c2), altstack = [sixthComponent, fifthComponent, fourthComponent]
		compute_third_component += pick(position=23,nElements=2)	# Pick b1
		compute_third_component += pick(position=13,nElements=2)	# Pick c2
		compute_third_component += fq2.mul(take_modulo=False,check_constant=False,clean_constant=False)

		# After this, the stack is: a1 b1 c1 d1 e1 a2 b2 c2 d2 e2, altstack = [sixthComponent, fifthComponent, fourthComponent, ((c1*b2) + (d1*e2) + (e1*d2) + (b1*c2))*xi]
		...
		# After this, the stack is: a1 b1 c1 d1 e1 a2 b2 c2 d2 (c1*e2), altstack = [sixthComponent, fifthComponent, fourthComponent, thirdComponent]
		compute_second_component = pick(position=15,nElements=2)	# Pick c1
		compute_second_component += fq2.mul(take_modulo=False,check_constant=False,clean_constant=False)

		# After this, the stack is: a1 b1 c1 d1 a2 b2 c2 d2 ((c1*e2) + (c2*e1))*xi, altstack = [sixthComponent, fifthComponent, fourthComponent, thirdComponent]
		...
		# After this, the stack is: a1 b1 c1 d1 a2 b2 c2 d2 ((c1*e2) + (c2*e1))*xi (a1*b2), altstack = [sixthComponent, fifthComponent, fourthComponent, thirdComponent]
		...
		# After this, the stack is: a1 b1 c1 d1 a2 b2 c2 d2 ((c1*e2) + (c2*e1))*xi (a1*b2) (a2*b1), altstack = [sixthComponent, fifthComponent, fourthComponent, thirdComponent]
		...
		# After this, the stack is: a1 b1 c1 d1 a2 b2 c2 d2, altstack = [sixthComponent, fifthComponent, fourthComponent, thirdComponent, ((c1*e2) + (c2*e1))*xi + (a1*b2)]
		compute_second_component += fq2.add_three(take_modulo=False,check_constant=False,clean_constant=False)
		compute_second_component += Script.parse_string('OP_TOALTSTACK OP_TOALTSTACK')
		...
		# After this, the stack is: a1 b1 d1 a2 b2 c2 (c1*d2), altstack = [sixthComponent, fifthComponent, fourthComponent, thirdComponent, secondComponent]
		compute_first_component = roll(position=11,nElements=2)	# Roll c1
		compute_first_component += fq2.mul(take_modulo=False,check_constant=False,clean_constant=False)

		# After this, the stack is: a1 b1 a2 b2 (c1*d2) (d1*c2), altstack = [sixthComponent, fifthComponent, fourthComponent, thirdComponent, secondComponent]
		compute_first_component += roll(position=9,nElements=2)	# Roll d1
		compute_first_component += Script.parse_string('OP_2ROT')	# Roll c2
		compute_first_component += fq2.mul(take_modulo=False,check_constant=False,clean_constant=False)

		# After this, the stack is: a1 a2 (c1*d2) (d1*c2) (b1*b2), altstack = [sixthComponent, fifthComponent, fourthComponent, thirdComponent, secondComponent]
		compute_first_component += Script.parse_string('OP_2ROT')	# Roll b2
		compute_first_component += roll(position=9,nElements=2)	# Roll b1
		compute_first_component += fq2.mul(take_modulo=False,check_constant=False,clean_constant=False)

		# After this, the stack is: a1 a2 ( (c1*d2) + (d1*c2) + (b1*b2) )*xi, altstack = [sixthComponent, fifthComponent, fourthComponent, thirdComponent, secondComponent]
	...
	def line_eval_times_eval_times_eval_times_eval_times_eval_times_eval(self,take_modulo: bool, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None, is_constant_reused: Optional[bool] = None) -> Script:
		'''
		...
		# After this, the stack is: a1 b1 c1 d1 e1 a2 b2 c2 d2 e2, altstack = [(d1*b2) + (e1*a2) + (a1*e2) + (b1*e2) + (c1*c2)]
		compute_sixth_component += fq2.add_three(take_modulo=False,check_constant=False,clean_constant=False)
		compute_sixth_component += fq2.add_three(take_modulo=False,check_constant=False,clean_constant=False)
		compute_sixth_component += Script.parse_string('OP_TOALTSTACK OP_TOALTSTACK')
		...
		# After this, the stack is: a1 b1 c1 d1 e1 a2 b2 c2 d2 e2 f2 (d1*a2) (e1*b2) (b1*f2), altstack = [sixthComponent]
		compute_fifth_component += pick(position=23,nElements=2)	# Pick b1
		compute_fifth_component += pick(position=7,nElements=2)	# Pick f2
		compute_fifth_component += fq2.mul(take_modulo=False,check_constant=False,clean_constant=False)

		# After this, the stack is: a1 b1 c1 d1 e1 a2 b2 c2 d2 e2 f2 (d1*a2) ( (e1*b2) + (b1*f2) + (c1*d2) )*xi, altstack = [sixthComponent]
		...
		# After this, the stack is: a1 b1 c1 d1 e1 a2 b2 c2 d2 e2 f2 (d1*a2) ( (e1*b2) + (b1*f2) + (c1*d2) )*xi (a1*e2), altstack = [sixthComponent]
		compute_fifth_component += pick(position=25,nElements=2)	# Pick a1
		compute_fifth_component += pick(position=9,nElements=2)	# Pick e2
		compute_fifth_component += fq2.mul(take_modulo=False,check_constant=False,clean_constant=False)

		# After this, the stack is: a1 b1 c1 d1 e1 a2 b2 c2 d2 e2 f2, altstack = [sixthComponent, (d1*a2) + ( (e1*b2) + (b1*f2) + (c1*d2) )*xi + (a1*e2)]
		compute_fifth_component += fq2.add_three(take_modulo=False,check_constant=False,clean_constant=False)
		compute_fifth_component += Script.parse_string('OP_TOALTSTACK OP_TOALTSTACK')
		...
		# After this, the stack is: a1 b1 c1 d1 e1 a2 b2 c2 d2 e2 f2 (d1*e2), altstack = [sixthComponent, fifthComponent]
		compute_fourth_component = pick(position=15,nElements=2)	# Pick d1
		compute_fourth_component += pick(position=5,nElements=2)	# Pick e2
		compute_fourth_component += fq2.mul(take_modulo=False,check_constant=False,clean_constant=False)

		# After this, the stack is: a1 b1 c1 d1 e1 a2 b2 c2 d2 e2 f2 (d1*e2) (e1*f2*xi), altstack = [sixthComponent, fifthComponent]
		compute_fourth_component += pick(position=15,nElements=2)	# Pick e1
		compute_fourth_component += pick(position=5,nElements=2)	# Pick f2
		compute_fourth_component += fq2.mul(take_modulo=False,check_constant=False,clean_constant=False)
		compute_fourth_component += fq2.mul_by_non_residue(take_modulo=False,check_constant=False,clean_constant=False)

		# After this, the stack is: a1 b1 c1 d1 e1 a2 b2 c2 d2 e2 f2 (d1*e2) (e1*f2*xi) (a1*d2), altstack = [sixthComponent, fifthComponent]
		compute_fourth_component += pick(position=25,nElements=2)	# Pick a1
		compute_fourth_component += pick(position=11,nElements=2)	# Pick d2
		compute_fourth_component += fq2.mul(take_modulo=False,check_constant=False,clean_constant=False)

		# After this, the stack is: a1 b1 c1 d1 e1 a2 b2 c2 d2 e2 f2 (d1*e2) (e1*f2*xi) (a1*d2) (b1*c2), altstack = [sixthComponent, fifthComponent]
		...
		# After this, the stack is: a1 b1 c1 d1 e1 a2 b2 c2 d2 e2 f2 (d1*e2) (e1*f2*xi) (a1*d2) (b1*c2) (c1*a2), altstack = [sixthComponent, fifthComponent]
		...
		# After this, the stack is: a1 b1 c1 d1 e1 a2 b2 c2 d2 e2 f2, altstack = [sixthComponent, fifthComponent, (d1*e2) + (e1*f2*xi) + (a1*d2) + (b1*c2) + (c1*a2)]
		compute_fourth_component += fq2.add_three(take_modulo=False,check_constant=False,clean_constant=False)
		compute_fourth_component += fq2.add_three(take_modulo=False,check_constant=False,clean_constant=False)
		compute_fourth_component += Script.parse_string('OP_TOALTSTACK OP_TOALTSTACK')
		...
		# After this, the stack is: a1 b1 c1 d1 e1 a2 b2 c2 d2 e2 f2 (d1*f2), altstack = [sixthComponent, fifthComponent, fourthComponent]
		compute_third_component = pick(position=15,nElements=2)		# Pick d1
		compute_third_component += Script.parse_string('OP_2OVER')	# Pick f2
		compute_third_component += fq2.mul(take_modulo=False,check_constant=False,clean_constant=False)

		# After this, the stack is: a1 b1 c1 d1 e1 a2 b2 c2 d2 e2 f2 (d1*f2) (e1*e2), altstack = [sixthComponent, fifthComponent, fourthComponent]
		compute_third_component += pick(position=15,nElements=2)	# Pick e1
		compute_third_component += pick(position=7,nElements=2)	# Pick e2
		compute_third_component += fq2.mul(take_modulo=False,check_constant=False,clean_constant=False)

		# After this, the stack is: a1 b1 c1 d1 e1 a2 b2 c2 d2 e2 f2 (d1*f2) (e1*e2) (b1*d2), altstack = [sixthComponent, fifthComponent, fourthComponent]
		compute_third_component += pick(position=23,nElements=2)	# Pick b1
		compute_third_component += pick(position=11,nElements=2)	# Pick d2
		compute_third_component += fq2.mul(take_modulo=False,check_constant=False,clean_constant=False)

		# After this, the stack is: a1 b1 c1 d1 e1 a2 b2 c2 d2 e2 f2 ( (d1*f2) + (e1*e2) + (b1*d2) + (c1*b2) )*xi, altstack = [sixthComponent, fifthComponent, fourthComponent]
		...
		# After this, the stack is: a1 b1 c1 d1 e1 a2 b2 c2 d2 e2 f2 ( (d1*f2) + (e1*e2) + (b1*d2) + (c1*b2) )*xi (a1*c2), altstack = [sixthComponent, fifthComponent, fourthComponent]
		compute_third_component += pick(position=23,nElements=2)	# Pick a1
		compute_third_component += pick(position=11,nElements=2)	# Pick c2
		compute_third_component += fq2.mul(take_modulo=False,check_constant=False,clean_constant=False)

		# After this, the stack is: a1 b1 c1 d1 e1 a2 b2 c2 d2 e2 f2, altstack = [sixthComponent, fifthComponent, fourthComponent, ( (d1*f2) + (e1*e2) + (b1*d2) + (c1*b2) )*xi + (a1*c2)]
		compute_third_component += fq2.add(take_modulo=False,check_constant=False,clean_constant=False)
		compute_third_component += Script.parse_string('OP_TOALTSTACK OP_TOALTSTACK')
		...
		# After this, the stack is: a1 b1 c1 d1 e1 a2 b2 c2 d2 e2 f2 (d1*c2), altstack = [sixthComponent, fifthComponent, fourthComponent, thirdComponent]
		...
		# After this, the stack is: a1 b1 c1 d1 e1 a2 b2 c2 d2 e2 f2 (d1*c2) (e1*d2), altstack = [sixthComponent, fifthComponent, fourthComponent, thirdComponent]
		...
		# After this, the stack is: a1 b1 c1 d1 e1 a2 b2 c2 d2 e2 (d1*c2) ( (e1*d2) + (c1*f2) )*xi, altstack = [sixthComponent, fifthComponent, fourthComponent, thirdComponent]
		...
		# After this, the stack is: a1 b1 c1 d1 e1 a2 b2 c2 d2 e2 (d1*c2) ( (e1*d2) + (c1*f2) )*xi (a1*b2), altstack = [sixthComponent, fifthComponent, fourthComponent, thirdComponent]
		...
		# After this, the stack is: a1 b1 c1 d1 e1 a2 b2 c2 d2 e2 (d1*c2) ( (e1*d2) + (c1*f2) )*xi (a1*b2) (b1*a2), altstack = [sixthComponent, fifthComponent, fourthComponent, thirdComponent]
		...
		# After this, the stack is: a1 b1 c1 d1 e1 a2 b2 c2 d2 e2, altstack = [sixthComponent, fifthComponent, fourthComponent, thirdComponent, (d1*c2) + ( (e1*d2) + (c1*f2) )*xi + (a1*b2) + (b1*a2)]
		compute_second_component += fq2.add_three(take_modulo=False,check_constant=False,clean_constant=False)
		compute_second_component += fq2.add(take_modulo=False,check_constant=False,clean_constant=False)
		compute_second_component += Script.parse_string('OP_TOALTSTACK OP_TOALTSTACK')
		...
		# After this, the stack is: a1 b1 d1 e1 a2 b2 c2 d2 (e2*c1), altstack = [sixthComponent, fifthComponent, fourthComponent, thirdComponent, secondComponent]
		compute_first_component = roll(position=15,nElements=2)	# Roll c1
		compute_first_component += fq2.mul(take_modulo=False,check_constant=False,clean_constant=False)

		# After this, the stack is: a1 b1 e1 a2 b2 c2 (e2*c1) (d1*d2), altstack = [sixthComponent, fifthComponent, fourthComponent, thirdComponent, secondComponent]
		compute_first_component += roll(position=13,nElements=2)	# Roll d1
		compute_first_component += Script.parse_string('OP_2ROT')	# Roll d2
		compute_first_component += fq2.mul(take_modulo=False,check_constant=False,clean_constant=False)

		# After this, the stack is: a1 b1 a2 b2 ( (e2*c1) + (d1*d2) + (e1*c2) ), altstack = [sixthComponent, fifthComponent, fourthComponent, thirdComponent, secondComponent]
		compute_first_component += Script.parse_string('OP_2ROT')	# Roll c2
		compute_first_component += roll(position=11,nElements=2)	# Roll e1
		compute_first_component += fq2.mul(take_modulo=False,check_constant=False,clean_constant=False)
		compute_first_component += fq2.add_three(take_modulo=False,check_constant=False,clean_constant=False)

		# After this, the stack is: a1 a2 ( (b1*b2) + (e2*c1) + (d1*d2) + (e1*c2) )*xi, altstack = [sixthComponent, fifthComponent, fourthComponent, thirdComponent, secondComponent]
		compute_first_component += Script.parse_string('OP_2SWAP')	# Roll b2
	...
	def miller_loop_output_square(self,take_modulo: bool, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None, is_constant_reused: Optional[bool] = None) -> Script:
		return MillerOutputOperations.square(self,take_modulo=take_modulo,check_constant=check_constant,clean_constant=clean_constant,is_constant_reused=is_constant_reused)
	
	def miller_loop_output_mul(self, take_modulo: bool, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None, is_constant_reused: Optional[bool] = None) -> Script:
		return MillerOutputOperations.mul(self,take_modulo=take_modulo,check_constant=check_constant,clean_constant=clean_constant,is_constant_reused=is_constant_reused)
	
	def line_eval_times_eval_times_miller_loop_output(self, take_modulo: bool, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None, is_constant_reused: Optional[bool] = None) -> Script:
		return self.line_eval_times_eval_times_eval_times_eval_times_eval_times_eval(take_modulo=take_modulo,check_constant=check_constant,clean_constant=clean_constant,is_constant_reused=is_constant_reused)
	
	def miller_loop_output_times_eval_times_eval_times_eval(self, take_modulo: bool, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None, is_constant_reused: Optional[bool] = None) -> Script:
		return MillerOutputOperations.mul(self,take_modulo=take_modulo,check_constant=check_constant,clean_constant=clean_constant,is_constant_reused=is_constant_reused)
	
	def miller_loop_output_times_eval_times_eval_times_eval_times_eval(self, take_modulo: bool, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None, is_constant_reused: Optional[bool] = None) -> Script:
		return MillerOutputOperations.mul(self,take_modulo=take_modulo,check_constant=check_constant,clean_constant=clean_constant,is_constant_reused=is_constant_reused)
	
	def miller_loop_output_times_eval_times_eval_times_eval_times_eval_times_eval_times_eval(self, take_modulo: bool, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None, is_constant_reused: Optional[bool] = None) -> Script:
		return MillerOutputOperations.mul(self,take_modulo=take_modulo,check_constant=check_constant,clean_constant=clean_constant,is_constant_reused=is_constant_reused)
