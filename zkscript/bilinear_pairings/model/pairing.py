...
class Pairing:
	...
	def single_pairing(self, modulo_threshold: int, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None) -> Script:
		'''
		Pairing computation.
		Input parameters:
		...
		Assuption on data:
			...
			- If P is the point at infinity, then it is encoded as 0x00 * N_POINTS_CURVE (NOT OP_0, data payloads of 0x00)
			- If Q is the point at infinity, then it is encoded as 0x00 * N_POINTS_TWIST (NOT OP_0, data payloads of 0x00)
		'''
		...
		out += hard_exponentiation(take_modulo=True,modulo_threshold=modulo_threshold,check_constant=False,clean_constant=clean_constant)
	...
	def triple_pairing(self, modulo_threshold: int, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None) -> Script:
		'''
		NOTE: At the moment, this function does not handle the case where one of the Pi's or one of the Qi's is the point at infinity
		...
		Assuption on data:
			...
			- [miller(P1,Q1) * miller(P2,Q2) * miller(P3,Q3)] is the product of the outputs of the miller loops computed on Pi,Qi
		'''
		...
		# After this, the stack is: quadratic([miller(P1,Q1) * miller(P2,Q2) * miller(P3,Q3)])^-1 quadratic([miller(P1,Q1) * miller(P2,Q2) * miller(P3,Q3)])
	...
	def single_pairing_input(self, P: list[int], Q: list[int], lambdas_Q_exp_miller_loop: list[list[list[int]]], miller_output_inverse: list[int], load_q = True) -> Script:
		'''
		Function that given P, Q, and the curve, returns the script needed to execute the single_pairing function above.
	...
	def triple_pairing_input(
			...
			load_q = True) -> Script:
		'''
		Function that given Pi, Qi, the lamdbas for computing (t-1)Qi, and the inverse of the miller loop, returns the script needed to execute the triple_pairing function above.
		'''
		q = self.MODULUS
