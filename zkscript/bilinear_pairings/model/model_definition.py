...
from zkscript.bilinear_pairings.model.miller_loop import MillerLoop
from zkscript.bilinear_pairings.model.triple_miller_loop import TripleMillerLoop
from zkscript.bilinear_pairings.model.pairing import Pairing
...
class PairingModel(MillerLoop, TripleMillerLoop, Pairing):
	...
	def __init__(
		...
		# Script for product of three line evaluations, assuming the first product has been calculated: the script computes ev * t1, where t1 = ev * ev
		self.line_eval_times_eval_times_eval = line_eval_times_eval_times_eval
		# Script for product of four line evaluations, assuming each couple of products has been calculated: the script computes t1 * t2, where t1 = ev * ev, t2 = ev * ev
		self.line_eval_times_eval_times_eval_times_eval = line_eval_times_eval_times_eval_times_eval
		# Script of product of six line evaluations, assuming each couple of products has been calculated: the script computes t1 * t4, where t1 = ev * ev, t4 = t2 * t3, t2 = ev * ev, t3 = ev * ev
		self.line_eval_times_eval_times_eval_times_eval_times_eval_times_eval = line_eval_times_eval_times_eval_times_eval_times_eval_times_eval
		# Script to compute the multiplication of the product of two line evaluations and the Miller output: the script computes t1 * t2, where t1 = ev * ev, t2 = miller_output
		...
		# Script to compute the multiplication of the Miller output with a product of three line evaluations: the script computes t1 * t2, where t1 = miller_output, t2 = ev * ev * ev
		self.miller_loop_output_times_eval_times_eval_times_eval = miller_loop_output_times_eval_times_eval_times_eval
		# Script to compute the multiplication of the Miller output with a product of six line evaluations: the script computes t1 * t2, where t1 = miller_output, t2 = ev * ev * ev * ev * ev * ev
		self.miller_loop_output_times_eval_times_eval_times_eval_times_eval_times_eval_times_eval = miller_loop_output_times_eval_times_eval_times_eval_times_eval_times_eval_times_eval
		...
		self.pad_eval_times_eval_times_eval_times_eval_to_miller_output = pad_eval_times_eval_times_eval_times_eval_to_miller_output
