...
class MillerLoop:
	...
	def miller_loop(self, modulo_threshold: int, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None) -> Script:
		'''
        ...
        Modulo operations to save space: notice that point calculations (doubling and addition) and updates of f (the return value of the Miller loop) come into contact
        ...
        To estimate the size increases, we use:
        	- log(f^2) <= log(13*3) + 2*sizeF
        	- log(f * lineEvaluation) <= log(13*3) + sizeF + log(q)
        	- log(f * lineEvaluation * lineEvaluation) <= 2*log(13*3) + sizeF + 2*log(q)
        	- P + Q has its worst computation at lambda verification: log(lambda * (xP - xQ)) <= log(q) + log(2) + log(max(xP,xQ)) (lambda is always assumed to be in Fq)
        	- P + Q has its worst coordinate at y: log(-y_P + (x_(P+Q) - x_P) * lambda) <= log(max(2yP, 2 * lambda * x_(P+Q)) <= log(max(2yP,4*x_(P+Q))) <= log(2*lambda*x_(P+Q)) <= log(6) + log(q) + log(max(xP,xQ)), where we used x_(P+Q) = lambda^2 - xP - xQ, and log(lambda^2 - xP - xQ) <= log(3*max(xP,xQ)) <= log(3) + log(max(xP,xQ)) (lambda is always assumed to be in Fq)
		'''
		...
		pad_eval_times_eval_times_eval_times_eval_to_miller_output = self.pad_eval_times_eval_times_eval_times_eval_to_miller_output
		...
		where f_i is the value of the Miller loop after the i-th iteration and lambda_2T is the gradient of the line tangent at T.
		If exp_miller_loop[i-1] != 0, then the stack is: lambda_(2T pm Q) lambda_(2T) P Q T f_i, where lambda_(2T pm Q) is the gradient of the line through 2T and pm Q.
		'''
		...
		for i in range(len(exp_miller_loop)-2,-1,-1):
			...
			if i == 0:
				...
				if exp_miller_loop[i-1] == 0:
					# In this case, the next iteration will have: f <-- f^2 * lineEvaluation and T <-- 2T. 
					future_size_F = log(13*3,2) + 2*current_size_F + log(13*3,2) + BIT_SIZE_Q
					...
					# In this case, the next iteration will have: f <-- f^2 * lineEvaluation * lineEvaluation and T <-- 2T \pm Q. 
					future_size_F =  log(13*3,2) + 2*current_size_F + 2*log(13*3,2) + 2*BIT_SIZE_Q
					if future_size_F > modulo_threshold:
						take_modulo_F = True
						current_size_F = BIT_SIZE_Q
					else:
			...
			if i == len(exp_miller_loop)-2:
				...
				if exp_miller_loop[i] == 0:
					...
					out += pick(position=(3*N_POINTS_TWIST + N_POINTS_CURVE + EXTENSION_DEGREE) + STACK_LENGTH_ADDED - 1,nElements=EXTENSION_DEGREE)				# Pick lambda_2T
					...
					out += point_doubling_twisted_curve(take_modulo=take_modulo_F,check_constant=False,clean_constant=False)										# Compute 2T
					STACK_LENGTH_ADDED = N_POINTS_TWIST
					# After this, the stack is: P Q -Q 2T lambda_2T T P
					out += roll(position=(3*N_POINTS_TWIST + N_POINTS_CURVE + EXTENSION_DEGREE) + STACK_LENGTH_ADDED - 1,nElements=EXTENSION_DEGREE)				# Roll lambda_2T
					STACK_LENGTH_ADDED += EXTENSION_DEGREE
					out += roll(position=N_POINTS_TWIST + STACK_LENGTH_ADDED - 1,nElements=N_POINTS_TWIST)															# Roll T
					STACK_LENGTH_ADDED += 0
					out += pick(position=(3*N_POINTS_TWIST + N_POINTS_CURVE) + STACK_LENGTH_ADDED - 1,nElements=N_POINTS_CURVE)										# Pick P
					STACK_LENGTH_ADDED += N_POINTS_CURVE
					# After this, the stack is P Q -Q 2T ev_(l_(T,T))(P)
					out += line_eval(take_modulo=True,check_constant=False,clean_constant=False,is_constant_reused=False)
					STACK_LENGTH_ADDED = N_ELEMENTS_EVALUATION_OUTPUT
					# After this, the stack is: P Q -Q 2T ev_(l_(T,T))(P)^2
					out += pick(position=N_ELEMENTS_EVALUATION_OUTPUT-1,nElements=N_ELEMENTS_EVALUATION_OUTPUT)														# Duplicate ev_(l_(T,T))(P)
					out += line_eval_times_eval(take_modulo=take_modulo_F,check_constant=False,clean_constant=False,is_constant_reused=False)
					...
					out += pick(position=(3*N_POINTS_TWIST + N_POINTS_CURVE + EXTENSION_DEGREE) + STACK_LENGTH_ADDED - 1,nElements=EXTENSION_DEGREE)				# Pick lambda_(2T pm Q)
					STACK_LENGTH_ADDED += EXTENSION_DEGREE
					out += pick(position=N_POINTS_TWIST + STACK_LENGTH_ADDED - 1,nElements=N_POINTS_TWIST)															# Pick 2T
					STACK_LENGTH_ADDED += N_POINTS_TWIST
					if exp_miller_loop[i] == 1:
						out += pick(position=3*N_POINTS_TWIST + STACK_LENGTH_ADDED - 1,nElements=N_POINTS_TWIST)													# Pick Q
						STACK_LENGTH_ADDED += N_POINTS_TWIST
					else:
					...
					out += roll(position=3*N_POINTS_TWIST + N_POINTS_CURVE + EXTENSION_DEGREE + STACK_LENGTH_ADDED - 1,nElements=EXTENSION_DEGREE)					# Roll lambda_(2T pm Q)
					STACK_LENGTH_ADDED += EXTENSION_DEGREE
					...
					out += pick(position=N_ELEMENTS_EVALUATION_TIMES_EVALUATION-1,nElements=N_ELEMENTS_EVALUATION_TIMES_EVALUATION)									# Duplicate ev_(l_(2T,\pm Q))(P) * ev_(l_(T,T))(P)
					out += line_eval_times_eval_times_eval_times_eval(take_modulo=take_modulo_F,check_constant=False,clean_constant=False,is_constant_reused=False)
				...
				if exp_miller_loop[i] == 0:
					...
					STACK_LENGTH_ADDED = 0
					out += pick(position=N_ELEMENTS_MILLER_OUTPUT + N_POINTS_TWIST + STACK_LENGTH_ADDED - 1,nElements=N_POINTS_TWIST)								# Pick T
					STACK_LENGTH_ADDED += N_POINTS_TWIST
					out += pick(position=N_ELEMENTS_MILLER_OUTPUT + 3*N_POINTS_TWIST + N_POINTS_CURVE + EXTENSION_DEGREE + STACK_LENGTH_ADDED - 1,nElements=EXTENSION_DEGREE)				# Pick lambda_2T
					STACK_LENGTH_ADDED += EXTENSION_DEGREE
					...
					out += pick(position=N_ELEMENTS_MILLER_OUTPUT + 3*N_POINTS_TWIST + N_POINTS_CURVE + STACK_LENGTH_ADDED - 1,nElements=EXTENSION_DEGREE)			# Pick P
					out += line_eval(take_modulo=True,check_constant=False,clean_constant=False,is_constant_reused=False)
					STACK_LENGTH_ADDED = N_ELEMENTS_EVALUATION_OUTPUT
					# After this, the stack is: lambda_(2T) P Q -Q T, altstack = [f_i^2 * ev_(l_(T,T))(P)]
					out += miller_loop_output_times_eval(take_modulo=take_modulo_F,check_constant=False,clean_constant=False,is_constant_reused=False)
					...
					if EXTENSION_DEGREE == 2 and N_POINTS_TWIST == 4:
						...
						out += roll(position=N_POINTS_TWIST + EXTENSION_DEGREE - 1,nElements=N_POINTS_TWIST)														# Roll T																								# Roll T
					out += point_doubling_twisted_curve(take_modulo=take_modulo_T,check_constant=False,clean_constant=clean_final)
					STACK_LENGTH_ADDED = 0
					...
					out += pick(position=N_POINTS_TWIST + STACK_LENGTH_ADDED - 1,nElements=N_POINTS_TWIST)															# Pick T
					STACK_LENGTH_ADDED += N_POINTS_TWIST
					out += point_doubling_twisted_curve(take_modulo=take_modulo_T,check_constant=False,clean_constant=False)
					STACK_LENGTH_ADDED = N_POINTS_TWIST
					# After this, the stack is: lambda_(2T \pm Q) lambda_(2T) P Q -Q T 2T (2T \pm Q), altstack = [f_i^2]
					out += pick(position=3*N_POINTS_TWIST + N_POINTS_CURVE + 2*EXTENSION_DEGREE + STACK_LENGTH_ADDED - 1,nElements=EXTENSION_DEGREE)				# Pick lambda_(2T pm Q)
					STACK_LENGTH_ADDED += EXTENSION_DEGREE
					...
					if exp_miller_loop[i] == 1:
						...
						out += pick(position=2*N_POINTS_TWIST + STACK_LENGTH_ADDED - 1,nElements=N_POINTS_TWIST)													# Pick -Q
					out += point_addition_twisted_curve(take_modulo=take_modulo_T,check_constant=False,clean_constant=False)
					STACK_LENGTH_ADDED = 2*N_POINTS_TWIST
					# After this, the stack is: lambda_(2T \pm Q) P Q -Q 2T (2T \pm Q) ev_(l_(T,T))(P), altstack = [f_i^2]
					out += roll(position=3*N_POINTS_TWIST + N_POINTS_CURVE + EXTENSION_DEGREE + STACK_LENGTH_ADDED - 1,nElements=EXTENSION_DEGREE)					# Roll lambda_2T
					STACK_LENGTH_ADDED += EXTENSION_DEGREE
					out += roll(position=N_POINTS_TWIST + STACK_LENGTH_ADDED - 1,nElements=N_POINTS_TWIST)															# Roll T
					STACK_LENGTH_ADDED += 0
					...
					# After this, the stack is: P Q -Q (2T \pm Q) ev_(l_(T,T))(P) ev_(l_(2T,\pm Q))(P), altstack = [f_i^2]
					out += roll(position=3*N_POINTS_TWIST + N_POINTS_CURVE + EXTENSION_DEGREE + STACK_LENGTH_ADDED - 1,nElements=EXTENSION_DEGREE)					# Roll lambda_(2T pm Q)
					STACK_LENGTH_ADDED += EXTENSION_DEGREE
					out += roll(position=N_POINTS_TWIST + STACK_LENGTH_ADDED - 1,nElements=N_POINTS_TWIST)															# Roll 2T
					STACK_LENGTH_ADDED += 0
					...
					out += line_eval_times_eval(take_modulo=False,check_constant=False,clean_constant=False)
					out += Script.parse_string(' '.join(['OP_FROMALTSTACK']*N_ELEMENTS_MILLER_OUTPUT))
					out += line_eval_times_eval_times_miller_loop_output(take_modulo=take_modulo_F,check_constant=False,clean_constant=clean_final,is_constant_reused=False)

		# After this, the stack is: (t-1)Q miller(P,Q)
		out += roll(position=N_ELEMENTS_MILLER_OUTPUT + 3*N_POINTS_TWIST + N_POINTS_CURVE - 1,nElements=2*N_POINTS_TWIST + N_POINTS_CURVE)
		out += Script.parse_string(' '.join(['OP_DROP'] * (2*N_POINTS_TWIST + N_POINTS_CURVE)))

		return optimise_script(out)

	def miller_loop_input_data(self, P: list[int], Q: list[int], lambdas_Q_exp_miller_loop: list[list[list[int]]]) -> Script:
		'''
		Function that takes q, exp_miller_loop, P and Q, lambdas_Q and returns the input data required to execute the function miller_loop above.
