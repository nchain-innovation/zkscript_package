...
class CyclotomicExponentiation:
	...
	def cyclotomic_exponentiation(self, exp_e: list[int], take_modulo: bool, modulo_threshold: int, check_constant: Optional[bool] = None, clean_constant: Optional[bool] = None) -> Script:
		'''
		Script to compute exponetiation f^e for f in the cyclotomic subgroup
		...
		count_prev = 0
		# At the end of this loop, the stack is : f .. f Inverse(f) ... Inverse(f) f ... f g according to the non-zero elements in exp_e
		...
		for i in range(len(exp_e)-2,-1,-1):
			...
			I am at the beginning of an iteration of the cycle and I assume that squaring will not raise an overflow error.
			Then, I check:
				- If exp_e[i] != 0:
					- Is squaring + multiplication raise an overflow?
						---> If yes, mod after squaring (no need to mod after multiplication because squaring + multiplication starting from bitSizeOfQ does not raise an overflow)
			...
			if i == 0 and take_modulo:
				...
				future_size = ceil(log(30,2)) + current_size * 2 				# After squaring
				future_size = ceil(log(30,2)) + future_size + BIT_SIZE_Q		# After squaring and multiplication

				if future_size > modulo_threshold:
					modulo_square = True 										# Mod out after squaring
					current_size = ceil(log(30,2)) + BIT_SIZE_Q  * 2			# After multiplying
				else:
					future_size_2 = ceil(log(30,2)) + future_size * 2			# After another squaring
			...
			if exp_e[i] != 0:
				...
				out += square(take_modulo=modulo_square,check_constant=False,clean_constant=False,is_constant_reused=False)
				out += mul(take_modulo=modulo_multiplication,check_constant=False,clean_constant=clean_constant_final,is_constant_reused=False)
			else:
				# After this, the stack is: f g^2
				out += square(take_modulo=modulo_square,check_constant=False,clean_constant=clean_constant_final,is_constant_reused=False)
